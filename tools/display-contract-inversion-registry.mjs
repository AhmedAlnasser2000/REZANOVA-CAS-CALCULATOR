...
export const OWNER_ASSEMBLY_REGISTRATIONS = [
  ...
  {
    id: 'equation-direct-trig-mathjson-assembly-v1',
    owner: 'equation-result-contract',
    rationale: 'The direct Trigonometry stage assembles typed Equation evidence before choosing the proven finite-root or canonical fallback adapter.',
    matchers: exact('src/lib/equation/guarded/direct-trig-stage.ts'),
  ...
  },
  {
    ...
    matchers: exact('src/lib/equation/guarded/polynomial-stage.ts'),
    functions: ['runBoundedPolynomialSolve'],
  },
  {
    id: 'equation-request-prep-mathjson-assembly-v1',
    owner: 'equation-result-contract',
    rationale: 'Equation request preparation rebuilds typed algebra and candidate-validation evidence before its unconditional final canonical adapter.',
    matchers: exact('src/lib/equation/guarded/request-prep.ts'),
    functions: ['attachAlgebraMetadata', 'validateDirectSymbolicOutcome'],
  },
  {
    id: 'equation-substitution-mathjson-assembly-v1',
    owner: 'equation-result-contract',
    rationale: 'Synchronous and asynchronous substitution stages assemble accepted and rejected candidate evidence before the shared canonical rebuild wrapper.',
    matchers: exact('src/lib/equation/guarded/substitution-stage.ts'),
    functions: ['substitutionSolve', 'substitutionSolveAsync'],
  },
  {
    id: 'equation-scoped-condition-mathjson-assembly-v1',
    owner: 'equation-result-contract',
    rationale: 'Scoped target finalization removes compatibility-only condition supplements and rebuilds typed canonical detail evidence in one final adapter.',
    matchers: exact('src/lib/modes/equation/outcomes.ts'),
    functions: ['withScopedTargetDependentConditions'],
  },
  {
    id: 'equation-parameterized-mathjson-assembly-v1',
    owner: 'equation-result-contract',
    rationale: 'Parameterized exponential, logarithmic, and Trigonometry routes assemble proven branch and supplement evidence before their final canonical adapter.',
    matchers: exact('src/lib/modes/equation/parameterized-result-outcomes.ts'),
    functions: ['createExpLogOrTrigOutcome'],
  },
  {
    id: 'equation-selected-target-mathjson-assembly-v1',
    owner: 'equation-result-contract',
    rationale: 'Selected-target parameterized routes assemble proven result evidence before the finite-root or canonical fallback adapter.',
    matchers: exact('src/lib/modes/equation/symbolic-parameterized-exact.ts'),
    functions: ['attachParameterizedSelectedTargetOutcome'],
  },
  {
    id: 'equation-symbolic-mathjson-assembly-v1',
    owner: 'equation-result-contract',
    rationale: 'The extracted symbolic helper assembles selected-target evidence before its unconditional final canonical adapter.',
    matchers: exact('src/lib/modes/equation/symbolic-result-outcomes.ts'),
    functions: ['createSelectedTargetIsolationOutcome'],
  },
  {
