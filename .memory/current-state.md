...
- Latest Limits frontier gate: `CALCULUS-LIMITS-GRUNTZ-REWRITE-W1` adds the internal rewrite-to-`w` contract on top of the strengthened MRV set: selected dominant atoms now produce `w` substitutions, residual-variable substitutions for basic invertible scales, transformed-expression evidence, parameter conditions, and real/principal-complex branch assumptions for later series/sign extraction. `CALCULUS-LIMITS-READBACK-SANITY2` remains a presentation/input-control gate over the visible Limit surface. No broad Gruntz solver route is exposed yet; the seed corpus remains ready for expansion after the future full Gruntz route.
