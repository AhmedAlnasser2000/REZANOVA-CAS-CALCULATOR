...
- Git posture: local `main` contains `f58cf1c0` (`CLIPBOARD-PIPELINE-RATCHET1`) and is one commit ahead of `origin/main` at `586795b0`; this session did not push. `DETAIL-SEGMENT-CONTRACT1` is implemented and verified in the current approved local checkpoint. No push is authorized; untracked `test-results/` remains unrelated and excluded.
...
- Presentation-roadmap posture: the user approved the Printer, canonical Clipboard, and typed detail-segment sequence recorded in `.memory/research/roadmaps/printer-detail-clipboard-roadmap.md`. Printer, additive payload, the 515-path TypeScript-AST migration ratchet, canonical Clipboard, and the shared copy/paste pipeline are committed through `f58cf1c0`. The current verified detail contract extracts one renderer shared by Display and Formula Viewer, gives nonempty typed parts precedence over explicit math/prose and legacy inference, adds typed solve-summary parts with derived compatibility text, and retains inference only for undeclared historical compatibility data. `DisplayResultBlocks.tsx` is 842 lines. The eight named detail migration slices start with Equation core; the mandatory pre-profile review and later structured-History/Display inversion remain locked. Standing commit approval applies to the named milestones in this session; no push is authorized.
...
Program sequence and acceptance details live in `.memory/research/roadmaps/anti-regression-nine-move-roadmap.md`. Behavioral Ratchet 9 is committed as `63d21229`; the complete closeout passed. The approved follow-up lives in `.memory/research/roadmaps/printer-detail-clipboard-roadmap.md`; its printer and Clipboard foundations are committed, the typed detail contract is verified in the current checkpoint, and `DETAIL-SEGMENT-EQUATION-CORE1` is next. No push is authorized.
