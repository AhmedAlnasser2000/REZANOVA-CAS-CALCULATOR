...
Language is now accepted as a future first-class product-text compartment, but not as an immediate translation wave. English remains the safe default/fallback for reset, invalid persisted settings, missing language resources, or failed validation. The future Language district should own prose, labels, commands, metadata, fallback, and interpolation; Display/notation should continue owning math rendering. Mixed prose plus math should move toward structured text/math parts so math fragments honor MathNotation, and RTL metadata must not blindly change math semantics. `LANGUAGE-SURFACE-AUDIT0` and `.memory/research/roadmaps/language-roadmap.md` set the next lane as foundation/pilot first: no solver wording rewrite, no Guide overhaul, no plugin/pack ecosystem, no schema churn beyond a later explicit settings seam, and no Graphing/Spreadsheet coupling.
