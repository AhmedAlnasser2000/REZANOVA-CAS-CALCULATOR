...
- Latest Limits frontier gate: `CALCULUS-LIMITS-GRUNTZ-ORCHESTRATION1` exposes Gruntz as a controlled Limit route after exact/domain/piecewise/abs/specialized symbolic routes and before numeric fallback for scale/indeterminate cases. Finite bridge cases such as one-sided `exp(1/x)` now surface compact Gruntz method and route evidence, while two-sided bridge disagreement stops with a controlled proof-style card. Plain Limit input also canonicalizes `exp(...)` into `e^{...}` for Gruntz-ready requests such as `exp(sqrt(x))/x^5`. Full unrestricted Gruntz remains future, and `npx tsc -b --pretty false` remains blocked by unrelated Equation complex-locus work.
