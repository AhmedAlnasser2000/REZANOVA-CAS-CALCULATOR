...
Workspace tabs have an invisible session identity layer, first active-only surface-state hosting, and OOE workspace-instance scoping after `WORKSPACE-INSTANCE-MODEL1`, `WORKSPACE-STATE-HOST1`, and `OOE-WORKSPACE-INSTANCE-SCOPE1`. The app still renders through the singleton `currentMode`, but app runtime shadows it with workspace instances carrying stable ids, mutable titles, compartment metadata, ordering, and future state slots. Core workspace surface state is hosted for Calculate, Equation, and Calculus only. OOE job identity, active/recent jobs, diagnostics records, lifecycle events, runtime envelopes, and launch-ticket evidence can now carry optional `workspaceInstanceId` plus launch-time labels; jobs from closed/missing instances stale-drop instead of late-committing. Display, committed History, persistence, and visible UI remain global. The agreed direction remains one app shell, no projects/files, no multiple `AppMain` copies, no second OOE authority, and no broad bus. Next tabs work should move to visible tab shell, mode-launcher tab commands, close/cancel UX, and pending-label/history polish.
