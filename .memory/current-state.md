...
`EQUATION-NUMERIC-INTERVAL-REVIVAL0` starts the next docs-only audit after semantics simplification. Numeric Interval Solve remains a route/tool, not a restored `Approx` answer mode. Current evidence shows ordinary numeric roots are stable, while dense/nested periodic and discontinuity cases are sensitive to endpoint shifts and subdivision count under the bracket-first sampler. Header Run/F1 still dispatches symbolic Equation solving even when the numeric panel is visible; the panel's inner `Run Numeric Solve` button is the separate numeric launcher. Recommended next order is route repair, guidance/suggested-interval handoff, then sampler stability.
