...
- Presentation-roadmap posture: the user approved the Printer, canonical Clipboard, and typed detail-segment sequence recorded in `.memory/research/roadmaps/printer-detail-clipboard-roadmap.md`. Internal compatibility/pedagogical printer profiles, hybrid exact Clipboard fallback, unchanged visible History, explicit live detail typing, risk-sliced migrations, and a separate later structured-History/Display-inversion roadmap are locked. Standing commit approval applies to the named milestones in this session; no push is authorized.
