...
- Git posture: `main` and `origin/main` currently meet at `586795b0` (`CLIPBOARD-CANONICAL1`) after `origin/main` advanced externally; this session did not push. `CLIPBOARD-PIPELINE-RATCHET1` is entering its approved local commit checkpoint. No push is authorized; untracked `test-results/` remains unrelated and excluded.
...
- Presentation-roadmap posture: the user approved the Printer, canonical Clipboard, and typed detail-segment sequence recorded in `.memory/research/roadmaps/printer-detail-clipboard-roadmap.md`. The pure printer substrate, additive payload, 515-path TypeScript-AST migration ratchet, and canonical envelope are committed through `586795b0`. `CLIPBOARD-PIPELINE-RATCHET1` now routes Display, Formula Viewer, History, Guide/workspace expressions, diagnostics, programmatic Paste, and native `MathEditor` events through shared adapters. `expressionRouting.ts` is the sole app-level paste authority; rich envelopes preserve canonical LaTeX, text-only input retains workspace canonicalization and Matrix/Vector naturalization, and direct production Clipboard API use outside `src/lib/clipboard/` is ratcheted to zero. AppMain is 3,306 lines with an exact lowered cap. Typed detail migration is next; the mandatory pre-profile review and later structured-History/Display inversion remain locked. Standing commit approval applies to the named milestones in this session; no push is authorized.
