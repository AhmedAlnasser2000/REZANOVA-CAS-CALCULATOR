...
- Git posture: `main` is four approved local commits ahead of `origin/main`: `db674b13` (`PRINTER-DETAIL-CLIPBOARD-ROADMAP0`), `f4cb2de2` (`PRINTER-SERIALIZATION-CONTRACT1`), `93e9b40e` (`DISPLAY-MATH-PAYLOAD1`), and `1b83f897` (`PRINTER-MIGRATION-RATCHET1`). `CLIPBOARD-CANONICAL1` is entering its approved commit checkpoint. No push is authorized; untracked `test-results/` remains unrelated and excluded.
...
- Presentation-roadmap posture: the user approved the Printer, canonical Clipboard, and typed detail-segment sequence recorded in `.memory/research/roadmaps/printer-detail-clipboard-roadmap.md`. The pure printer substrate, additive payload, and 515-path TypeScript-AST migration ratchet are committed through `1b83f897`. `CLIPBOARD-CANONICAL1` adds a bounded versioned math envelope, browser custom MIME plus escaped HTML and visible-text representations, and exact canonical fallback. The official Tauri v2 plugin has only read-text, write-text, and write-html authority; live Linux WebView evidence exposes HTML plus exact UTF-8 text and no custom MIME. Display Copy Result now preserves hidden canonical LaTeX while keeping visible notation in a successful rich browser item. AppMain is 3,348 lines with an exact lowered cap. Formula Viewer, History, Guide/workspace expressions, diagnostics, programmatic Paste, native `MathEditor` events, and the duplicated AppMain/expression-routing reads remain the explicit `CLIPBOARD-PIPELINE-RATCHET1` scope. Typed detail migration, the mandatory pre-profile review, and later structured-History/Display inversion remain locked. Standing commit approval applies to the named milestones in this session; no push is authorized.
