...
- Completed `EQUATION-PARAM6` as the direct affine trig selected-target parameterized Equation slice:
  - added `src/lib/equation/equation-parameterized-trig.ts` and focused tests
  - solves selected-target forms such as `sin(z)=a`, `cos(z+a)=b`, `tan(2z+a)=b`, `2sin(z+a)+c=d`, and `sin(A z+B)=c`
  - emits periodic selected-target families with integer parameter `n`
  - honors the active angle unit: RAD uses inverse trig plus `2πn`/`πn`, DEG uses `(180/π)` conversion plus `360n`/`180n`, and GRAD uses `(200/π)` conversion plus `400n`/`200n`
  - preserves range facts, symbolic nonzero coefficient facts, and `n\in\mathbb{Z}`
  - keeps multiple trig carriers, trig identities, nonlinear selected-target arguments, nested trig/deep `COMP`, raw adjacent products, variable memory, named string variables, `POLY-ELIM2`, graphing, source mirrors, and broad transcendental algebra out of scope
  - primary_agent: `codex`
