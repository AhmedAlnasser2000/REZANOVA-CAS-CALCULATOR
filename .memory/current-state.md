...
- Latest Limits frontier gate: `CALCULUS-LIMITS-GRUNTZ-FOUNDATION1` adds formal pre-Gruntz contracts for MRV-set extraction, comparability classes, rewrite-to-`w`, and sign/limit extraction fixtures. `CALCULUS-LIMITS-READBACK-SANITY2` is a later presentation gate over that surface, not a new solver route. No broad Gruntz solver route is exposed yet; the seed corpus remains ready for expansion after the future full Gruntz route.
