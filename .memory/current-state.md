...
- Latest Limits frontier gate: `CALCULUS-LIMITS-GRUNTZ-RECURSIVE-EVALUATOR1` adds an internal bounded recursive Gruntz evaluator over the existing MRV-set, rewrite-to-`w`, series-in-`w`, and sign-extraction contracts. Direct sign extraction still gets first chance, then capped exponential exponent and exponential-quotient cleanup can recursively resolve residuals such as `e^x/e^(x+log(x))`, `e^(e^x)/e^(e^x+x)`, and parameterized `e^(a*x)` cases. Principal-branch and parameter evidence are threaded forward, but no public Gruntz Limit route is exposed yet; finite-target bridging, orchestration, and seed corpus hardening remain ahead. `npx tsc -b --pretty false` remains blocked by unrelated Equation complex-locus work.
