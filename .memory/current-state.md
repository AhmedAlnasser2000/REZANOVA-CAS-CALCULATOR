...
- Variable values posture: `.memory/research/roadmaps/variable-values-and-substitution-roadmap.md` now owns stored numeric variable values and substitution policy. `VARIABLE-MEMORY1` should add a dedicated Variables side panel, finite real numeric stored values, visible standard-Calculate substitution, snapshot-based history replay, and no Equation symbolic substitution.
