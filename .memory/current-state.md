...
Program sequence and acceptance details live in `.memory/research/roadmaps/anti-regression-nine-move-roadmap.md`. All four Incident Closure moves, the mandatory review, the Linear Algebra transition, and Behavioral Ratchets 5-7 are committed. Behavioral Ratchet 8 is implemented and verified as warning-only operational freshness reporting. Standing commit approval covers Moves 6-9 but does not authorize a push. `HISTORY-REPLAY-RATCHET1` is the final move, and Matrix/Vector feature expansion remains frozen through its manual closeout.
