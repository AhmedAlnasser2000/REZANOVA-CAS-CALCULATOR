...
`EQUATION-SPECIAL-FORM-ROOTS-FRONTIER1` extends real Exact affine selected-target powers through degree 12 while keeping Complex power isolation capped at degree 4. It also adds an internal/test-traced `special-form-roots` selected-target family for exact-rational pure-power carrier quadratics such as `x^6-5x^3+4=0` and `x^{12}-5x^6+4=0`, with total degree `2n <= 12`. Symbolic carrier coefficients, affine/non-pure carrier substitution, broader special forms, visible implicit roots, numeric fallback as Exact closure, Display/History schemas, OOE/app-state/Tauri changes, graphing, step-by-step, and DAG/search graphs remain out of scope.
Post-frontier bugfix: live Equation input now normalizes unbraced multi-digit pasted/typed powers such as `^12` into `^{12}` before app-state solving, and single-target real Exact special-form carrier equations route before the older shared symbolic fallback so they do not return partial roots. Complex high-degree special-form cases still stop honestly because the frontier widened only real Exact.
