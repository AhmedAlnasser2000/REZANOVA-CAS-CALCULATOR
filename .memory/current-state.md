...
- OOE posture: `.memory/research/architecture/ooe-rust-first-evaluation.md` and `.memory/research/roadmaps/ooe-rust-first-roadmap.md` record Order Of Execution as a valid Rust-first kernel execution-order contract for capabilities, hosts, phases, stages, budgets, result stability, and traces. First OOE work should be `OOE-RS0` readiness/docs and then `OOE-RS1` Rust schema plus pure validation; no runtime routing, solver changes, UI changes, Progressive Solver, chunk scheduler, checkpoint ledger, or cancellation wiring should be bundled into the first gates.
