...
- OOE posture: `.memory/research/architecture/ooe-rust-first-evaluation.md`, `.memory/research/architecture/ooe-rs0-readiness-audit.md`, and `.memory/research/roadmaps/ooe-rust-first-roadmap.md` record Order Of Execution as a valid Rust-first execution traffic-control contract for capabilities, hosts, phases, stages, priorities, budgets, cancellability, stale-result policy, result stability, and traces. `OOE-RS0` is implemented as architecture/readiness only; `OOE-RS1` adds the first Rust OOE schema and pure plan validation under `src-tauri/src/ooe/`; `OOE-RS2` adds a Rust-only built-in plan registry for the six current kernel capabilities; `OOE-RS3` exposes narrow Tauri diagnostics commands to list built-in descriptors, fetch a built-in plan, and validate a submitted plan as a report object; `OOE-RS4` adds a bridge-only TypeScript diagnostics adapter with zod boundary parsing and unavailable web-preview fallbacks; `OOE-RS5` through `OOE-RS8` add fail-open internal pilots for Equation, standard Calculate expression routes, and active Table builds; `OOE-RS9` unifies those pilots behind a shared `{ payload, ooe }` runtime envelope while consumers still commit only payloads; `OOE-RS10` adds a tooling-only boundary validator so OOE production files cannot drift into UI, app-controller, Playground, source-mirror, `.memory`, Labs runner, or broad solver/runtime dependencies outside the explicit pilot allowlist. No user-visible routing, solver changes, UI changes, Progressive Solver, chunk scheduler, checkpoint ledger, MCP bridge, trace buffer, cancellation wiring, or Rust execution is included in these gates. Later OOE work should explicitly support optimization, traceability, developer diagnostics, and local read-only MCP observability.
