...
- Latest Limits frontier gate: `CALCULUS-LIMITS-GRUNTZ-MRV-SET2` strengthens the formal pre-Gruntz MRV-set contract with internal comparability classes, target-free symbolic coefficient drivers, real/principal-complex branch assumptions, and evidence rows for later rewrite-to-`w` and sign extraction. `CALCULUS-LIMITS-READBACK-SANITY2` remains a presentation/input-control gate over the visible Limit surface. No broad Gruntz solver route is exposed yet; the seed corpus remains ready for expansion after the future full Gruntz route.
