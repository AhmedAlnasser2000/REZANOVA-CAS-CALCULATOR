...
- Post equation parameterized-solving roadmap capture; `.memory/research/roadmaps/equation-parameterized-solving-roadmap.md` now sequences selected-target parameterized Equation solving as `EQUATION-PARAM1` through `EQUATION-PARAM7`, with affine/linear target isolation first and no jump to broad transcendental algebra.
...
- Completed `EQUATION-PARAM5` as the bounded exp/log selected-target parameterized Equation slice:
  - added `src/lib/equation/equation-parameterized-exp-log.ts` and focused tests
  - solves selected-target forms such as `e^z=a`, `e^(z+a)=b`, `ln(z+a)=b`, `log(z+a)=b`, `2^(z+a)=b`, `e^(z+a)=e^b`, and `ln(z+a)=ln(b)`
  - delegates generated equations to `EQUATION-PARAM1`, `EQUATION-PARAM2`, `EQUATION-PARAM3`, or `EQUATION-PARAM4` where safe, including `ln(z^2+a)=b`, `ln(1/(z-a))=b`, and `e^(|z-a|)=b`
  - preserves positive-output, log-domain, denominator, and delegated branch facts
  - keeps symbolic bases, log-combine sums/quotients, Lambert W, nested exp/log towers, variable memory, named string variables, `POLY-ELIM2`, graphing, source mirrors, and broad transcendental algebra out of scope
  - primary_agent: `codex`
