...
`ANSWER-READBACK-POLICY-AUDIT0` records that recurring exact-answer readability issues such as double imaginary units, reducible zero terms, noisy scalar factors, and awkward branch roots are final-answer readback normalization gaps before Display, not reasons to make Display a CAS. Solver routes keep math evidence, facts, validation, and stops; Symbolic Primitives keep bounded mechanics; Equation readback should normalize safe root/branch expressions before `DisplayOutcome`; Display stays responsible for rendering, layout, notation, and fail-closed branch extraction. Recommended next implementation is `ANSWER-READBACK-NORMALIZATION1`, with copy/history/editor canonical behavior decided explicitly before changing schemas or broad outputs.
