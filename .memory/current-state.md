...
`SYMBOLIC-SIMPLIFICATION-SURFACE-AUDIT0` confirms simplification should not start as final-answer prettification or a universal reducer. The next safe implementation is a private structural MathJSON simplification primitive whose first consumer is `src/lib/symbolic-engine/primitives/factorization/node-helpers.ts`; radical, power/log, Algebra exact-rational, Complex branch/readback, and final-answer polish surfaces remain separately owned. The grouped-factor readability case `x*(x+a)+b*(x+a)=0` is recorded as later route/readback polish because the factorization primitive can recognize the structure but current app route order may still let generic quadratic output claim the answer first.
