...
- Geometry boundary posture: `GEOMETRY-BOUNDARY0` confirms Geometry remains a visible guided geometry workspace rather than a redundant Calculate or Equation surface. Geometry owns geometric objects, measurements, coordinate workflows, and bounded solve-missing experiences, while broad equation solving stays in Equation, quick scalar evaluation stays in Calculate, and trig triangle-relation workflows stay in Trigonometry. Geometry OOE runtime-shell and launch-ticket adoption remain deferred until `GEOMETRY-REQUEST1` and `GEOMETRY-HISTORY1` provide a typed `geometrySeed` replay contract over the existing `GeometryRequest` union.
