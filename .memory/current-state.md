...
- Latest Limits frontier gate: `CALCULUS-LIMITS-BRANCH-DOMAIN-PROOFS2` standardizes compact proof cards for Piecewise disagreement, absolute-value side behavior, complex proof-first stops, and real-domain boundary stops, including Playwright visual evidence for `lim x -> 0 sqrt(x)` and `lim x -> 0 |x|/x`.
