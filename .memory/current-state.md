...
- Linear Algebra runtime seam posture: Matrix and Vector keep separate OOE identities through `linearAlgebra.matrix` and `linearAlgebra.vector` while production still shares `linear-algebra-worker-runtime`. App-facing dispatch, paste canonicalization, active-operand request construction, named snapshots, and Equation handoff types are exposed through `src/lib/linear-algebra/runtime-request.ts`; the compartment manifest treats that facade as public and enforces the workspace runtime-request boundary. The measured current-risk audit found only 1.34% Matrix and 5.00% Vector gzip asset reductions, effectively equal browser lifecycle timing, sub-2x serialization differences, and identical fallback/cancel/stale/History policies. After reviewing that truthful failed gate, the user prospectively locked independent Matrix and Vector hosts for the approved numerical-decomposition/conditioning versus exact/geometric arcs. Split implementation is now the next milestone; no remaining capability work may precede it.
...
Program sequence and acceptance details live in `.memory/research/roadmaps/anti-regression-nine-move-roadmap.md`. All four Incident Closure moves and the mandatory nine-workspace verification sweep are complete and user-accepted. `LINEAR-ALGEBRA-SHELL-SPLIT0` did not meet its current-risk threshold, but the user separately locked prospective independent hosts; `MATRIX-VECTOR-RUNTIME-SHELL-SPLIT1` is next. Behavioral Ratchets have not started, and Matrix/Vector feature expansion remains frozen through their Move 9 closeout.
