...
- Completed `EQUATION-PARAM7` as selected-target parameterized Equation readback/replay polish:
  - added shared readback helpers for PARAM1 through PARAM6 detail sections and safe supplement normalization
  - preserved existing family titles while standardizing selected-target and symbolic-parameter wording
  - added optional `equationSolveTarget` to history and Guide launch payloads so selected-target examples and history replay restore the intended target
  - refreshed Equation Guide examples for affine, quadratic, rational, carrier, exp/log, and trig parameterized solving
  ...
  - keeps solver families, result origins, badges, variable memory, named string variables, `POLY-ELIM2`, graphing, source mirrors, and broad CAS behavior unchanged
  - primary_agent: `codex`
