...
Solver capability work is now pointed back at search discipline before new algorithms. A corrected Claude handoff is preserved at `.memory/research/roadmaps/equation-solver-search-discipline-handoff.md`, and the dedicated roadmap is `.memory/research/roadmaps/equation-search-discipline-roadmap.md`. `EQUATION-SEARCH-DISCIPLINE-AUDIT0` records that the live slow-path risk is selected-target isolation plus exp/log generated-equation handoff into symbolic polynomial work, not a missing flat polynomial core or a semantic-planner-only problem. After `EQUATION-TARGET-SHAPE-PROFILE1`, Equation has a pure target-shape seam at `src/lib/equation/target-shape/` with a root facade at `src/lib/equation/equation-target-shape.ts`. After `EQUATION-SELECTED-TARGET-ROUTER-PERF1`, selected-target routing consumes that profile in both top-level parameterized routing and generated-equation handoff delegation, conservatively skipping impossible family attempts while preserving existing successes, stops, source labels, and readback behavior. After `EQUATION-SEARCH-TRACE-EVIDENCE1`, optional internal trace recorders can show top-level and generated-handoff profile summaries, skipped families, attempted families, winning families, and final stop reasons in tests without changing Display, History, OOE diagnostics, app-state, or persistence. After `EQUATION-GENERATED-HANDOFF-PERF1`, exp/log generated-equation solving uses a focused generated-handoff helper that profiles each generated equation, route-gates only the exp/log helper's supported local families (`linear`, `polynomial`, `rational`, `carrier`), and records internal trace evidence while preserving exact/readback semantics. After `POLYNOMIAL-SYMBOLIC-COEFF-SEAM1`, `src/lib/equation/parameterized/symbolic-polynomial.ts` owns the shared Equation MathJson-symbolic degree-2 coefficient seam consumed by parameterized polynomial and rational solving, while exact-rational `polynomial-core` remains a sibling/inspiration rather than a dependency target. After `EQUATION-GENERATED-BRANCH-HANDOFF-SEAM1`, carrier, composition, and mixed-algebraic generated branch solvers share an internal route-gated branch delegation seam while keeping branch generation, facts, fallback wording, source labels, readback, and result semantics local to each solver. After `EQUATION-MATHJSON-ARITHMETIC-ADOPTION1`, carrier and mixed-algebraic use the shared `math-json.ts` arithmetic helpers while preserving carrier parse/latex behavior and mixed-algebraic composition simplification/latex behavior; linear arithmetic remains deliberately local for a later parity audit. Next search-discipline work can consider cap recalibration, Cardano/Ferrari, broad factoring, integration breadth, graphing, step-by-step, or Rust migration. The Exact/Isolate answer-mode boundary is deliberately deferred until after search discipline; the likely future differentiator is `Isolate` as rearrangement/isolation intent and `Exact` as isolation plus root, branch, principal-range, domain, and candidate-validation semantics when it claims a solved answer.
