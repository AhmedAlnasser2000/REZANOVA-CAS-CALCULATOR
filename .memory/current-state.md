...
- Guide education-platform posture: current Guide remains the shipped in-app reference/example workspace. The future direction is an authorable educational platform built from notebooks, guidance packs, learner copies, compact computation/evidence snapshots, and safe import/export contracts. This is not a universal generated step-by-step engine and does not depend on embedding external textbooks or websites.
...
- Symbolic integration posture: current Calculus integration covers the agreed exact-rational and target-free symbolic Tier-I/Rubi surface, bounded Risch-Norman and Rothstein-Lazard-Rioboo-Trager heuristics, named special-function certificates, canonical genus-0/genus-1 algebraic slices, and the July 4 recognition/readback fix for early textbook indefinite integrals. The Calculus benchmark scaffold now starts with an indefinite-only integration corpus under `benchmarks/calculus-corpus/integration/`, with 200 Thomas first-batch unique cases, duplicate-to-canonical rules, visual-status run records, and validation coverage. Current focused evidence records all 200 backend-supported with no unclear readback patterns; broad symbolic partial fractions, broad generic algebraic genus-1 coefficient solving, definite elliptic evaluation, and deeper benchmark hardening remain future.
