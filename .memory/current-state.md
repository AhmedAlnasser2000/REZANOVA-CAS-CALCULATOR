...
- Multivariable posture: `AREA-MULTIVAR0` recommends `VARIABLE-CORE1` before Equation target UI, variable memory, `POLY-ELIM2`, or product-facing multivariable solving; Calcwiz must distinguish solve targets, stored numeric variables, symbolic parameters, active variables, and bound variables first. Identifier handling is conservative for now: `K` and `k` are distinct, while coding-style string variables such as `hello` are deferred and adjacent letters should behave as multiplication/ambiguity rather than one named variable. Reserved-token semantic highlighting is future visible UX work; `VARIABLE-CORE1` should provide internal classifications only.
