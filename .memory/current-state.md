...
- Anti-regression canary posture: `WORKSPACE-CANARY-SUITE1` defines a launcher-derived 19-case Playwright floor across Calculate, Equation, Calculus, Trigonometry, Geometry, Statistics, Matrix, Vector, and Table. The verified browser run passes 19/19 in 74.90 seconds; direct structured Statistics requests avoid the separately deferred guided-control defects. Calculate exact inverse trig now preserves bounded special values across DEG/RAD/GRAD, and the canary incident also closed exact Equation affine-root and Geometry circle-pi readback defects.
...
- Linear Algebra input posture: successful Matrix/Vector editor parses canonicalize pasted list syntax and MathLive matrix variants into natural matrix/vector LaTeX for editor, preview, history, and replay; keyboard paste and app `Paste` naturalize supported friendly list imports immediately before Run, while raw list syntax remains import-only and malformed paste stays editable for controlled Run errors. Matrix typed editor expressions can compose arbitrary named or inline matrices before routing into existing Matrix operations. Vector typed editor expressions can compose arbitrary named or inline vectors for add/subtract, norm/unit/angle/Gram-Schmidt, general `proj(base,target)`, exact 3D cross product, and scalar triple product. Soft F-keys remain active Left/Right or First/Second shortcuts only. The named-value libraries have readable dark controls, active operand badges, insert-name-to-editor buttons, and large-grid card layout polish. Structured Linear Algebra answer rows may improve display readability while `exactLatex` remains the copy/export truth.
- Linear Algebra runtime seam posture: Matrix and Vector keep separate OOE identities through `linearAlgebra.matrix` and `linearAlgebra.vector` while sharing `linear-algebra-worker-runtime`. App-facing dispatch, paste canonicalization, active-operand request construction, named snapshots, and Equation handoff types are exposed through `src/lib/linear-algebra/runtime-request.ts`; the compartment manifest treats that facade as public and enforces the workspace runtime-request boundary. Worker splitting remains evidence-gated rather than feature-count-driven.
...
- Guide and Notebook education-platform posture: Guide is a singleton app-level page surface outside the calculator shell, with existing reference/example content rehosted through the page-surface model and null Order of Execution runtime context. The July 5 content/evidence gates make Statistics intentionally Guide-visible, repair Trigonometry/Statistics domain article parity for `trig-period-phase` and `statistics-inference`, strengthen Guide content reachability and launch-payload tests, and add representative Guide-page Playwright click-through evidence for Calculate, Equation, Calculus, Trigonometry, Statistics, Geometry, Matrix, and Vector examples. Notebook V1 is now a document-tab app page outside the calculator shell: each Notebook tab owns a session draft with versioned DTOs, math-aware text blocks, non-destructive detected/accepted inline math spans, rich text marks, optional standalone MathLive editor blocks, a workspace selector with live Calculate/Equation handoff only, and compact evidence/package boundary placeholders. Public overview source wording is caught up through the Guide gates but may need a later catch-up for Notebook V1. Teacher/community packages, import/export, restart persistence, in-notebook solving, History insertion, grading, cloud/community hosting, Graphing, Spreadsheet, and Guide persistence remain future; this is not a universal generated step-by-step engine and does not depend on embedding external textbooks or websites.
...
Program sequence and acceptance details live in `.memory/research/roadmaps/anti-regression-nine-move-roadmap.md`. All four Incident Closure moves are verified; the mandatory nine-workspace user review is next, and Behavioral Ratchets remain blocked until that checkpoint is accepted.
