...
- Latest Limits frontier gate: `CALCULUS-LIMITS-GRUNTZ-FINITE-TARGET-BRIDGE1` adds an internal finite-target Gruntz bridge that maps finite one-sided approaches through `x=a+1/t` or `x=a-1/t` into positive-infinity recursive Gruntz contracts, preserves the original direction, and stops two-sided limits when bridge-side results disagree. The bridge reuses the recursive evaluator and threads parameter/principal-branch evidence forward, but no public Gruntz Limit route is exposed yet; orchestration and seed corpus hardening remain ahead. `npx tsc -b --pretty false` remains blocked by unrelated Equation complex-locus work.
