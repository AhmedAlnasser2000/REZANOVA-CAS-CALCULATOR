...
- Latest Limits frontier gate: `CALCULUS-LIMITS-GRUNTZ-SERIES-IN-W1` adds internal leading-term extraction after rewrite-to-`w`: transformed expressions now produce a leading `w` order/coefficient contract with propagated parameter conditions, branch assumptions, and compact evidence rows for later sign/limit extraction. `CALCULUS-LIMITS-READBACK-SANITY2` remains a presentation/input-control gate over the visible Limit surface. No broad Gruntz solver route is exposed yet; the seed corpus remains ready for expansion after the future full Gruntz route.
