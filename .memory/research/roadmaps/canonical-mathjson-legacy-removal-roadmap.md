...
Status: active; verified through `DISPLAY-DETAIL-LEGACY-CLOSEOUT1`; no push is authorized
...
- `CANONICAL-OUTCOME-CALCULATE-EQUATION1`: verified. Calculate, Equation, and Equation's direct-symbolic helper now cross worker boundaries as validated `CanonicalRuntimeOutcome`; Calculate and Equation OOE jobs carry the same canonical payload before their current public wrappers derive Display. Equation's private analysis evidence remains bounded host metadata for provenance rather than entering the result contract. The accepted inversion inventory has 647 producer observations, 613 consumers, 174 native documents, one legacy-History compatibility projection, 411 legacy reads, and no violations. Focused worker/result tests, TypeScript, lint, build, OOE/compartment/file-size gates, and post-build Chromium evidence preserve output and overflow. `CANONICAL-OUTCOME-SYMBOLIC-CALCULUS1` is next.
- `CANONICAL-OUTCOME-SYMBOLIC-CALCULUS1`: verified. The one Calculus workspace shell carries derivative, integral, limit, series, transform, partial, ODE, and IVP results across worker, fallback, cancellation, and OOE boundaries as validated `CanonicalRuntimeOutcome`; native Symbolic/Calculus IR and the public Display shape remain unchanged. The runtime-contract matrix exposed and closed a prior Calculate projection defect: canonical-only results now retain `exactLatex` without inventing `canonicalMath`. The accepted inversion inventory has 645 producer observations, 613 consumers, 174 native documents, one legacy-History compatibility projection, 411 legacy reads, and no violations. Focused tests, 77 runtime contracts, static gates, and fresh Chromium Calculus/canonical-only Calculate evidence pass. `CANONICAL-OUTCOME-GUIDED-DOMAINS1` is next.
- `CANONICAL-OUTCOME-GUIDED-DOMAINS1`: verified. Trigonometry, Geometry, and Statistics workers, fallbacks, cancellation paths, and OOE jobs now carry validated canonical runtime outcomes plus their established parsed/replay metadata as sibling host transport. Public guided wrappers derive their unchanged Display payload only after OOE completion and intentionally avoid adding a duplicate `canonicalMath` field; the canonical document retains every proven tree. Invalid worker completions fail closed. The accepted inversion inventory has 648 producer observations, 613 consumers, 174 native documents, one legacy-History compatibility projection, 411 legacy reads, and no violations. Focused worker/result/UI tests, 20 runtime probes, 80 runtime contracts, static boundaries, and six zero-retry Chromium canaries pass after one isolated first-load Settings-panel miss was reproduced cleanly. `CANONICAL-OUTCOME-LINEAR-ALGEBRA-TABLE1` is next.
- `CANONICAL-OUTCOME-LINEAR-ALGEBRA-TABLE1`: verified. Matrix and Vector preserve independent workers, hosts, capabilities, shells, requests, fallback hosts, and replay ownership while their shared lifecycle client carries validated canonical payloads. Their malformed post-start completions fail closed. Table carries canonical result truth beside its structured response evidence and retains cooperative fallback for worker failures, including malformed completions; cancellation now owns a canonical error document with no partial rows. Public mode wrappers project the same Display shapes only after OOE completion. The accepted inversion inventory improves to 641 producer observations, 613 consumers, 176 native documents, one legacy-History compatibility projection, 411 legacy reads, and no violations. Focused tests, 21 probes, 82 runtime contracts, static boundaries, six canaries, and three node-level visual captures pass without output drift. `CANONICAL-CONSUMER-DIRECT1` is next.
- `CANONICAL-CONSUMER-DIRECT1`: verified. The default consumer resolver now requires a validated native canonical document; a separately named resolver contains the one old-History display projection. Clipboard, workspace `Ans` state, diagnostics, replay, print hygiene, and Surface DTO mapping no longer recover semantic truth from compatibility strings. Missing authority fails closed. The accepted inventory is 642 producers, 611 consumers, 176 native documents, one compatibility projection, 410 legacy reads, and zero violations. Focused contracts, all 100 replay fixtures, 75 result-contract tests, 39 Surface tests, 60 clipboard tests, 33 History/UI state tests, five representative Chromium checks, and one inspected production screenshot pass without output drift. `HISTORY-CANONICAL-ONLY1` is next.
- `HISTORY-CANONICAL-ONLY1`: verified. New History rows persist one required validated `CanonicalResultDocumentV1` plus replay identity, context, and seeds; legacy result fields and the old-History projection are gone. Browser and Tauri ledgers atomically remove old or malformed V1 rows with one non-blocking count notice, while future-version rows remain hidden, byte-preserved, and excluded from V1 retention and clear operations. Rows over the append budget retry once with optional MathJSON removed and `canonical-only-fallback`; canonical structure and LaTeX are unchanged, and a second overflow fails without truncation. The accepted inversion inventory is 642 producers, 597 consumers, 177 native documents, zero compatibility projections, 396 registered legacy reads, and zero violations. Focused backend/UI tests, 100 replay fixtures, Rust persistence tests, real Chromium reload/failure/preferences/cleanup checks, and inspected Equation History/replay screenshots pass. `DISPLAY-DETAIL-LEGACY-CLOSEOUT1` is next.
- `DISPLAY-DETAIL-LEGACY-CLOSEOUT1`: verified. Live Display derives detail lines and solve summaries from validated canonical documents; `solveSummaryText`, legacy detail inference, and live compatibility recovery are removed. All 452 live detail producers declare typed math/text intent, the Display lane has zero legacy reads, and the accepted inventory is 641 producers, 595 consumers, zero compatibility projections, 177 native documents, and 393 registered legacy reads outside Display. Authorized broader app-runtime verification exposed and closed two latent boundary defects: Calculate advisories no longer emit explicit `undefined`, and inline Matrix profile proofs use the owned exact matrix MathJSON rather than treating formatted matrix LaTeX as a symbol. Sixty app-logic and 142 runtime UI tests, 76 result-contract tests covering all 43 golden and 100 replay executions, TypeScript, build, lint, file-size, inversion, compartments, and four Chromium detail surfaces pass. Test-only Display fixture bridges remain isolated for `RESULT-COMPATIBILITY-REMOVAL1`; concurrent Notebook files and `test-results/` remain excluded.
