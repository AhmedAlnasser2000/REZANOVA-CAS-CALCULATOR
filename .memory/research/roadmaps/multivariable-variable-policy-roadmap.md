...
- `EQUATION-ISOLATION1` implements one-island target-free shell isolation without creating `PARAM16`, two-island algebra, or Equation symbolic stored-value substitution.
...
- `EQUATION-ISOLATION1` now removes target-free algebra around one selected-target island before handing off to existing selected-target solvers, while leaving symbolic cube-root/power isolation and two-island algebra for future milestones.
