...
`EQUATION-TARGET1` is now available as the first visible solve-target foundation for Equation mode. `EQUATION-PARAM1` through `EQUATION-PARAM5` now cover bounded selected-target affine/linear, quadratic, rational, nonperiodic carrier, and exp/log inverse-pair families while preserving non-target symbols as symbolic parameters.
