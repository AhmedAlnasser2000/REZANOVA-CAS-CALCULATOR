...
Status: active; full Calculus ownership is committed as `ed29552a`; Guided Domains is verified through Gate 14 and entering its approved commit; `RESULT-DOCUMENT-LINEAR-ALGEBRA1` is next; no push is authorized
...
- `RESULT-DOCUMENT-GUIDED-DOMAINS1`: verified and entering its approved commit. Trigonometry, Geometry, and Statistics attach producer-owned canonical truth at their final runtime owner boundaries; Table attaches it only after a completed structured response. Prompts and Table cancellation remain control-only, and executable Geometry transfer actions remain transient.
...
- The live mixed checkout's full unit, UI, and lint failures are confined to concurrent uncommitted Notebook section work and are excluded from this commit. TypeScript, production Vite build, owned lint, file size, printer/detail/clipboard, runtime, CI, seam, Surface, OOE, compartment, and diff gates pass. Long guided request-preview formulas have pre-existing narrow-card clipping outside the result card and are tracked separately. Linear Algebra is next.
