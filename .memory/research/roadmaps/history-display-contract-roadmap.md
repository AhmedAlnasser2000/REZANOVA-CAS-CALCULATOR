...
Status: active; Symbolic Integration is committed as `392fb45b`; full Calculus owner coverage is verified through Gate 13 and entering its approved commit; `RESULT-DOCUMENT-GUIDED-DOMAINS1` is next; no push is authorized
...
- `RESULT-DOCUMENT-CALCULUS1`: verified and entering its approved commit. The explicit native-screen policy now covers all 16 result screens: derivatives, derivative at point, implicit derivative, the three Limit screens, the three integral screens, Maclaurin, Taylor, Laplace, partial derivative, both symbolic ODE screens, and Numeric IVP. Seven home/navigation screens remain control-only and cannot accidentally become result producers.
