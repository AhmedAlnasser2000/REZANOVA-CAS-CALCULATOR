...
## What Changed
- Generalized the shared square-root rationalization/conjugate profile in `src/lib/algebra/radical-core.ts` so supported exact denominator families now include:
  - `a + b\sqrt{u}`
  - `b\sqrt{u} + c\sqrt{v}`
...
- Normalized explicit transform supplements through `src/lib/algebra/algebra-transform.ts` so widened conjugate/rationalize output stays on the `POLY-RAD4` grouped `Exclusions:` / `Conditions:` path.
