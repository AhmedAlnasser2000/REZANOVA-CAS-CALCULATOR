# SYMBOLIC-FACTORIZATION-FACADE-PARITY1 Verification Summary
...
- Primitive-backed path is limited to `shared-carrier-grouping`; legacy facade fallback remains.
- Focused primitive/facade tests passed.
- Focused Equation parity tests passed.

## Commands

- `npm run test:unit -- src/lib/symbolic-engine/primitives/factorization/factorization.test.ts src/lib/symbolic-engine/factoring.test.ts src/lib/symbolic-engine/mixed-factor.test.ts`
  - Passed: 3 files, 25 tests.
- `npm run test:unit -- src/lib/equation/parameterized/factorable-polynomial.test.ts src/lib/equation/roots/readback.test.ts src/lib/equation/facts/branch-domain-facts.test.ts src/lib/modes/equation/parameterized-search-trace.test.ts`
