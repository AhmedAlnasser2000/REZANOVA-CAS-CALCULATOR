...
## Verification
- PASS: `npm run test:unit -- src/lib/calculus/workspace/limits.test.ts src/lib/symbolic-engine/limits/finite-leading-terms.test.ts src/lib/calculus/limit-route-corpus.test.ts src/lib/calculus/limit-route-classifier.test.ts`
...
## Notes
- Direct probes before implementation showed the requested examples already resolved to `1`; this milestone strengthened the explanation surface and locked the examples in tests.
- `approxText` remains numeric for exact finite known-rule results such as `1/2`, while `exactLatex` owns the rendered exact answer.
