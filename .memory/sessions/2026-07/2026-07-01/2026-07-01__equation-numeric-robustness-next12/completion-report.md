...
- `EQUATION-COMPLEX-REGION-NONLINEAR-SOLVE1`: added visible Complex nonlinear numeric solving behind an explicit bounded rectangular region request and an explicit Equation Complex Region panel. The route runs only after supported exact and Complex polynomial fallback routes miss, protects the solve target while substituting stored non-target values for explicit region solves, uses principal-branch diagnostics, deterministic seed-grid Newton, and contour/winding verification, and reports local region confidence without claiming global Complex completeness. The UI/runtime surface keeps Complex Region separate from Real Numeric Interval, makes the explicit panels mutually exclusive, and preserves OOE stale/cancel checks through a distinct `complex-region` route.
