...
- Every new successful result producer and every new math-bearing controlled-error producer must build a validated `CanonicalResultDocumentV1` through its workspace-owned adapter before the final runtime boundary. It must pass `requireCanonicalResultAuthority`, `npm run test:display-contract-inversion`, and `npm run test:result-contract`. Do not add a compatibility registration, broad forwarding exemption, or string-only result path to make a new producer pass.
...
- When proven answer MathJSON is unavailable, keep canonical LaTeX without inventing a tree and record the missing producer proof when that gap matters to the milestone. Optional MathJSON coverage may grow only through producer-owned evidence; absence is safer than false structure.
...
- If a new capability needs result structure not representable by `CanonicalResultDocumentV1`, extend and version the canonical result contract first. Do not smuggle new semantics through titles, prose, detail strings, metadata labels, or canonical LaTeX alone.
