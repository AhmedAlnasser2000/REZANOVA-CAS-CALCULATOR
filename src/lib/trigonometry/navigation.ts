...
const ROUTE_META: Record<TrigScreen, TrigRouteMeta> = {
  ...
  equationSolve: {
    ...
    description: 'Solve bounded equations such as sin(x)=1/2, sin(2x)=0, sin(x)cos(x)=1/2, 2cos^2(x)-1=0, or tan^2(x)-1=0 through the shared trig editor.',
    helpText: 'Enter a supported equation in x in the top editor, then press EXE or F1. Selected exact trig rewrites, square forms, and single-carrier substitution families are reduced automatically before solve. Use Send Eqn when you need interval-based numeric solving.',
    guideArticleId: 'trig-equations',
    focusTarget: 'editor',
    editorMode: 'editable',
  },
