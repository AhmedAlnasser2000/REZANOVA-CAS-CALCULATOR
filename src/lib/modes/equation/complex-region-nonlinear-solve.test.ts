...
describe('Equation Complex region nonlinear solve', () => {
  ...
  it('returns exact locus evidence instead of analytic contour solving for recognized non-holomorphic carriers', () => {
    const cases = [
      {
        equationLatex: String.raw`\left|z-1\right|=2`,
        exactLatex: String.raw`\left|z-1\right|=2`,
        expected: 'This equation describes a circle in the complex plane.',
      },
      {
        equationLatex: 'Re(z)=1',
        exactLatex: String.raw`z=1+t\imaginaryI`,
        expected: 'This is a vertical line in the complex plane.',
      },
      {
        equationLatex: 'Im(z)=1',
        exactLatex: String.raw`z=t+\imaginaryI`,
        expected: 'This is a horizontal line in the complex plane.',
      },
      {
        equationLatex: 'conj(z)=z',
        exactLatex: 'z=t',
        expected: 'A complex number equals its conjugate exactly on the real axis.',
      },
      {
        equationLatex: 'abs(z)=0',
        exactLatex: 'z=0',
        expected: 'A zero magnitude fixes one complex point.',
      },
    ];

    for (const { equationLatex, exactLatex, expected } of cases) {
      ...
      if (result.kind !== 'success') {
        throw new Error(`Expected exact locus result for ${equationLatex}`);
      }
      const text = collectOutcomeText(result);
