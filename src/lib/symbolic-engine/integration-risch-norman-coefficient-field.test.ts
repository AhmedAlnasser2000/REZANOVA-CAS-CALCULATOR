...
function coefficient(latex: string, variable = 'x') {
  const result = parseRischNormanCoefficient(node(latex), variable);
...
describe('Risch-Norman coefficient field', () => {
  ...
  it('builds scoped arithmetic and denominator facts', () => {
    const a = coefficient('a');
    const b = coefficient('b');
    const c = coefficient('c');

    const sum = addRischNormanCoefficients(a, b, 'x');
    ...
    const product = multiplyRischNormanCoefficients(sum.coefficient, c, 'x');
    ...
    const difference = subtractRischNormanCoefficients(sum.coefficient, a, 'x');
    ...
    const quotient = divideRischNormanCoefficients(product.coefficient, sum.coefficient, 'x');
  ...
  it('exposes zero and one helpers', () => {
    const zero = zeroRischNormanCoefficient('x');
    const one = oneRischNormanCoefficient('x');
    ...
    expect(isRischNormanCoefficientZero(zero.coefficient)).toBe(true);
    expect(isRischNormanCoefficientOne(one.coefficient)).toBe(true);
  });

  it('rejects coefficients outside the first Risch-Norman scope', () => {
