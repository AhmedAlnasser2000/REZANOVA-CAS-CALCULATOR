...
type GeometricMeasureExpression = Extract<LinearAlgebraEditorExpression, { kind: 'geometricMeasure' }>;
...
export function dispatchVectorGeometricExpression(
  expression: GeometricMeasureExpression,
  input: VectorFamilyDispatchInput,
): VectorFamilyDispatchResult {
  const operands = [];
  for (const operandExpression of expression.operands) {
  ...
  const length = operands[0]?.vector.length ?? 0;
  if (operands.some((operand) => operand.vector.length !== length)) {
    return { ok: false, message: 'All geometric-measure vectors must have the same length.' };
  }

  const first = operands[0];
  const second = operands[1];
  if (!first || !second) {
    return { ok: false, message: 'Geometric measures require at least two vectors.' };
  }
  const allExact = operands.every((operand) => operand.exactVector);
  return {
    ok: true,
