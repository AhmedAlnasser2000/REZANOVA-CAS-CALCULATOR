  ...
  if (rows !== columns) {
    ...
    return attachLinearAlgebraCanonicalEvidence(response, {
      ...
      details: [
        ...
        math(`\\operatorname{pivotColumns}(${input.label})=\\{${pivotColumnsLatex(pivotColumns)}\\}`, equationMathJson(operatorMathJson('pivotColumns', operand), integerSetMathJson(pivots)), 'matrix.invertibility.native-pivots'),
      ],
    });
  }
