...
describe('Notebook rich surface state', () => {
  ...
  it('keeps version-5 state and migrates version-1 state losslessly', () => {
    ...
    expect(migrated.document.version).toBe(5);
  ...
  it('migrates version-4 documents by changing only their version', () => {
    ...
    expect(migrated.document).toEqual({ ...version4, version: 5 });
  });
});
