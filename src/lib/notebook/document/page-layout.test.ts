...
describe('Notebook page geometry', () => {
  ...
  it('reports the strictest physical or rendered width that can truthfully wrap', () => {
    const setup = {
      paperSize: 'a4' as const,
      orientation: 'portrait' as const,
      marginsPt: { ...NOTEBOOK_MARGIN_PRESETS_PT.normal },
    };
    const physical = notebookMaximumWrappedMediaWidthPercent(setup);
    const rendered = notebookMaximumWrappedMediaWidthPercent(setup, 480);

    expect(physical).toBeCloseTo(57.454, 3);
    expect(rendered).toBeCloseTo(46.25, 3);
    expect(notebookEffectiveImagePlacement(setup, 'square-left', rendered, 480))
      .toBe('square-left');
  });
