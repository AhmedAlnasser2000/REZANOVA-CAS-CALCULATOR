...
function cycleAngleUnit(angleUnit: AngleUnit): AngleUnit {
  return angleUnit === 'deg' ? 'rad' : angleUnit === 'rad' ? 'grad' : 'deg';
}

function isAnyFormTarget(target: EventTarget | null) {
  const element = target as HTMLElement | null;
  return !!element?.closest('input, select, button, textarea, math-field');
}

function isPlainFormTarget(target: EventTarget | null) {
  const element = target as HTMLElement | null;
  return !!element?.closest('input, select, button, textarea');
}

function emptySystem(size: 2 | 3) {
  return Array.from({ length: size }, () => Array.from({ length: size + 1 }, () => 0));
}

function polynomialTemplateLatex(view: PolynomialEquationView) {
  switch (view) {
    case 'quadratic':
      return 'ax^2+bx+c=0';
    case 'cubic':
...
function guideSoftActionLabel(action: GuideSoftAction) {
